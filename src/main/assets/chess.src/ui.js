...
// v1.2.3 round-47 (S1481): removed the unused _cachedCount/_totalSteps/
//   _allCached locals — nothing here reads them anymore; the analyze-all
//   button logic lives in _buildRvAnalyzeBtnHTML() (which computes its own
//   copies), so only the call below remains.
...
function _computeInitialPositionAnnotations(){
  ...
  try{
    ...
    if(!st)return;
    // We compute annotations from BOTH sides' perspectives (the threat
    // detection below examines both directions for blue/yellow arrows).
    // No red/green check arrows are generated for the initial position
    // (no move was just made).
    // v1.2.3 round-47 (S1481/S1854): removed the unused moverColor/oppColor
    //   locals — the annotation logic below works off playerCol/aiCol and
    //   per-piece colors instead.

# Building Regalia v1.0.6 from source
...
- v1.0.6 (2026.6.27): Major feature additions — Chess960 ECO suppression, PGN
  `[SetUp]`/`[FEN]` round-trip preservation, stats.html per-move selection,
  unified gray-out styling, portrait-optimized new game settings, scroll-position
  preservation, engine-eval FEN sanitization, king-then-rook castling gesture.
- The Stockfish 18 `arm64-v8a-dotprod` binary is unchanged from v1.0.4 (NDK r27c
  build, 114 MB, NEON+dotprod acceleration).
- If you modify any file in `chess.src/`, re-run `bash build-chess.sh` before
  `./gradlew assembleRelease` to ensure the latest JS is bundled into the APK.
